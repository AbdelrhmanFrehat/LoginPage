...
class _HomeViewState extends State<HomeView> {
  ...
  void initState() {
    super.initState();
    isDarkMode = false; // ✅ لتجنب الخطأ
    _loadThemeMode();
  }

  void _loadThemeMode() async {
    // اقرأ القيمة من التخزين أو Firebase أو أي مصدر آخر
    final storedValue = false; // مثلاً
    setState(() {
  ...
  Future<List<Course>> _fetchCourses(String teacherId) {
    return CourseApi().readAll(teacherId); // تأكد أن readAll تقبل teacherId
  }
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          CircleAvatar(
            ...
            child: const Text("TS", style: TextStyle(color: Colors.black)),
          ),
      ...
      children: [
        ...
        FutureBuilder<List<Course>>(
          future: _fetchCourses(
            context.watch<AuthenticationViewModel>().teacher.id.toString(),
          ),
